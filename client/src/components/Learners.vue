				...
				<template v-slot:body="props">
					...
					>
						<q-td
							...
							:props="props"
							:key="col.name"
							>{{ getColumnValueDesc(col.name, col.value) }}</q-td
						>
					</q-tr>
				</template>
			</q-table>
...
export default {
  ...
  methods: {
    rowClicked(event, props) {
      ...
      this.selectedLearner = this.data.filter((object) => object.student_number == props.row.student_number)[0];
    },
  ...
  mounted() {
    ...
    this.$axios
      ...
      .then((response) => {
        ...
        this.visibleColumns = this.columns.map((column, index) => (index = this.columns.length ? column.name : ''));
